...
capabilities. There is a lack of mechanisms for state monitoring, discovery and coordination across massive Agents.
...
There is no common way to manage the agent itself, to let a human safely interact with it, or to let multiple agents from
...
## Example: Intervention During a Multi-Controller Operation

An operator terminates a remediation task after an agent has submitted changes to two network controllers. One controller has applied its change; the other is still processing the request. Reporting the agent task as "terminated" does not tell the operator which changes took effect, whether further actions are prevented, or whether recovery is required. A common supervision interface needs to distinguish the intervention request, its effective outcome, and the status of the associated network operations.
...
Based on the whole lifecycle of a NMA, it may introduce operational problems for network management Agents running in live production environments, e.g.,
...
there is no tamper-evident record of what was revoked, when, and by whom — undermining both security posture and regulatory
compliance.
...
Based on these two perspectives, the following operational problems may arise:
...
## End-to-End O&M Operations Executed by Multi-Vendor Agents
...
fragile, non-reproducible, and auditably opaque. Figure 1 shows the task delegation and evidence message flow between
different agents.
...
contained without enforceable risk controls—each additional hop across agents and administrative domains amplifies the inconsistency, ultimately rendering multi-agent collaboration incapable of being executed reliably, safely, or auditably end to end.
...
several of the problems described in this draft: task-based authorization, emergency break-off and rollback control, and task risk
...
Thanks to XXX for valuable comments and feedback.
